...
- Template support is intentionally small: string substitution only for `.Namespace.Name`, `.Namespace.UID`, `.Namespace.Labels.<key>`, `.Namespace.Annotations.<key>`, and `.Class.Name`, with no loops, conditionals, functions, external lookups, or cross-resource references.
